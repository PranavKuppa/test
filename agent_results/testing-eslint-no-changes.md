...
Detected **1** lint/compliance issue(s); automatically resolved **0**.

**1** issue(s) had no safe automatic fix and are left for your review:

- `(eslint):0` eslint-fatal (error) — eslint did not run successfully (exit code 2); its findings could NOT be checked, so this is not a clean result (Error: --config:). A missing or misnamed eslint configuration is the most common cause.

---

⚠️ Tests pass and no changes were made, but 1 issue have no automatic fix and need your review, see details above.
